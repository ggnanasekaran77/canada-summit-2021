...
def call() {
    ...
    def jobDirName = new File("${JOB_NAME}")
    def pipelineMap = new URL ("http://configserver:8888/pipelinemap/default/main/${jobDirName}.yml").getText()
    def pipelineYaml = readYaml text: pipelineMap
    pipelineYaml['pipelineName'] = pipelineYaml?.pipelineName?:"defaultPipeline"

    log.info "cicd jobDirName ${jobDirName}"
